    ...
    public GameObject[] Generate() {
        ...
        for (int y = 0; y < countY; y++) {
            for (int x = 0; x < countX; x++) {
                hits[x, y] = IsHit(new Vector2(bottomLeft.x + x, bottomLeft.y + y + 0.2f));
            }
        }
        ...
        if (removePreviouslyGenerated) {
            int childs = shadowCastersContainer.transform.childCount;
            for (int i = childs - 1; i >= 0; i--) {
                GameObject.DestroyImmediate( shadowCastersContainer.transform.GetChild( i ).gameObject );
            }
        }
